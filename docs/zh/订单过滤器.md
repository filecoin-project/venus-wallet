...
3. BlockHeader 签 blockHeader 出块使用
...
7. SignedVoucher 签 Paychannel 信息 市场使用
...
使用 jq 脚本来运行订单过滤 需要先安装 jq 命令 [jq install](https://stedolan.github.io/jq/download/).
...
使用 javascript 脚本来运行订单过滤,相对于 jq 脚本，用 js 脚本可以直接编辑，并且立即生效。 需要先安装 node 工具 [nodejs install](https://nodejs.org/en/download/).
